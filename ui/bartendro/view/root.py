...
def index():
    if app.globals.get_state() == fsm.STATE_ERROR:
        return render_template(
            ...
            other_drinks=[],
            error_message=
            "Bartendro is in trouble!<br/><br/>I need some attention! Please find my master, so they can make me feel better.",
    ...
    if (not len(top_drinks) and not len(other_drinks)) or app.globals.get_state() == fsm.STATE_HARD_OUT:
        return render_template(
            ...
            other_drinks=[],
            error_message=
            "Drinks can't be made with the available boozes.<br/><br/>I need some attention! Please find my master, so they can make me feel better.",
            title="Bartendro error")
    ...
    return render_template("index",
                           ...
                           lucky_drink_id=can_make[int(random.randint(0,
                                                                      len(can_make) - 1))],
                           title="Bartendro")
...
def shots():
    ...
    if len(shots) == 0:
        return render_template(
            ...
            options=app.options,
            error_message=
            "Bartendro is out of all boozes. Oh no!<br/><br/>I need some attention! Please find my master, so they can make me feel better.",
            title="Bartendro error")

    return render_template("shots", num_shots_ready=len(shots), options=app.options, shots=shots, title="Shots")
