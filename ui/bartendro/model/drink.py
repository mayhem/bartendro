...
class Drink(db.Model):
    ...
    def set_ingredients_text(self, txt=""):
        self.ingredients = [{'name': txt, 'id': 0, 'parts': 1, 'type': 0}]
    ...
    def process_ingredients(self):
        ...
        for db in self.drink_boozes:
            ing.append({'name': db.booze.name, 'id': db.booze.id, 'parts': db.value, 'type': db.booze.type})
    ...
    def __repr__(self):
        return "<Drink>(%d,%s,%s,%s)>" % (self.id or -1, self.name.name, self.desc, " ".join(
            ["<DrinkBooze>(%d)" % (db.id or -1) for db in self.drink_boozes]))
